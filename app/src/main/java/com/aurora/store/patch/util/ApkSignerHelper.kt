...
object ApkSignerHelper {
    ...
    private fun getKeystore(context: Context): KeyStore {
        ...
        val exportKeystoreFile = File(
            Environment.getExternalStorageDirectory(),
            "auroraStore/keystore/keystore.p12"
        )

        publicKeystoreFile.parentFile?.mkdirs()

        if (exportKeystoreFile.canRead() &&
            exportKeystoreFile.exists() &&
            exportKeystoreFile.isFile &&
            exportKeystoreFile.length() > 0
        ) {
            try {
                val tempFile = File(publicKeystoreFile.parent, "keystore.tmp")

                exportKeystoreFile.inputStream().use { input ->
                ...
                tempFile.copyTo(publicKeystoreFile, overwrite = true)
                tempFile.delete()
        ...
        if (!publicKeystoreFile.exists() || publicKeystoreFile.length() == 0L) {
            ...
            FileOutputStream(publicKeystoreFile).use {
                ks.store(it, PASSWORD)
            }
        }

        runCatching {
            if (!exportKeystoreFile.exists() || exportKeystoreFile.length() == 0L) {
                exportKeystoreFile.parentFile?.mkdirs()
                publicKeystoreFile.copyTo(exportKeystoreFile, overwrite = true)
            }
        }

        publicKeystoreFile.inputStream().use {
            ks.load(it, PASSWORD)
        }
    ...
    fun getCertificate(context: Context): X509Certificate {
        val cert = getKeystore(context).getCertificate(ALIAS) as X509Certificate
        return cert
    }
